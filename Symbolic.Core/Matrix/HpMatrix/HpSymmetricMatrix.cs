...
namespace Calcpad.Core
{
    internal class HpSymmetricMatrix : HpMatrix
    {
        ...
        // Umbral: matrices >= EigenThreshold usan Eigen C++ (SparseLU/LDLT)
        private const int EigenThreshold = 100;

        internal override HpVector LSolve(HpVector vector)
        {
            // Para matrices grandes: usar Eigen C++ (SparseLU con fallback LDLT)
            if (_rowCount >= EigenThreshold && EigenInterop.IsAvailable())
                return EigenSolve(vector);
        ...
        {
            // Para matrices grandes: usar Eigen C++ (SimplicialLDLT con fallback SparseLU)
            // ClSolve: uses Eigen for n >= EigenThreshold
            if (_rowCount >= EigenThreshold && EigenInterop.IsAvailable())
                return EigenSolve(v);
        ...
        // Solver Eigen C++: extrae formato skyline, llama DLL, retorna vector solución
        private HpVector EigenSolve(HpVector v)
        {
            // Eigen C++ skyline solver path
            EigenInterop.ExtractSkyline(this, out int[] rowSizes, out double[] values);

            // Extraer RHS (right-hand side)
            var rhs = new double[_rowCount];
            for (int i = 0; i < _rowCount; i++)
                rhs[i] = v.GetValue(i);

            var x = new double[_rowCount];
            int result = EigenInterop.skyline_cholesky_solve(_rowCount, rowSizes, values, rhs, x);
