...
namespace Calcpad.Core
{
    internal class MatrixCalculator
    {
        ...
        new Dictionary<string, int>()
        {
            ...
            { "cells", 43 },
            // ── Calcpad Lab: aliases MATLAB ──────────────────────────────
            { "eye", 0 },        // MATLAB eye(n) → identity(n)
            { "transpose", 20 }, // MATLAB transpose(A) → transp(A)
            { "inv", 22 },       // MATLAB inv(A) → inverse(A)
            // det ya existe con el mismo nombre
            // rank ya existe con el mismo nombre
            // trace ya existe con el mismo nombre
        ...
        new Dictionary<string, int>()
        {
            ...
            { "matmul", 35 },
            // ── Calcpad Lab: aliases MATLAB ──────────────────────────────
            { "zeros", 0 },      // MATLAB zeros(m,n) → matrix(m,n) (Create con default 0)
            // (ones se mapea aparte porque necesita fill — ver MatlabBuiltins)
            // size, length, mldivide, mldivide_lsolve etc se manejan en MatlabBuiltins
        }.ToFrozenDictionary(StringComparer.OrdinalIgnoreCase);
